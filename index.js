...
//imprt google authnntication file
  ...
  await User.findOne({email:req.body.uemail},async function(err,doc){
    if(!err && doc!==null && doc.verify===1)
    {
      // if user is verified then it come into this block and comparePassword
        doc.comparePassword(req.body.upass, function(err, isMatch) {
          if(err)
          {
...
//redireact to main page
app.get('/main',isSession,function(req,res){
  let json=JSON.parse(req.session.views);
  res.render('Main',{username:json.name});
});
...
app.get('/google/callback', passport.authenticate('google', {
  ...
  function(req, res) {
    ...
    res.redirect('/check-credentials');
  }
);
      ...
      await User.findOne({email:req.user.email},async function(err,doc){
        if(err===null && doc===null)
        {
          let newInstance=new User({
            ...
            await newInstance.save(function(error)
          {
            //send mail to the user that he/she is Successfully isLoggedIn
            if(!error)
            {
              req.session.views = JSON.stringify({email:req.user.email,name:req.user.displayName});
                 res.redirect('/main');
              //send mail
            }
          });
...
//error
req.session.views = JSON.stringify({email:doc.email,name:doc.name});
   res.redirect('/main');
        }
    });
  }
